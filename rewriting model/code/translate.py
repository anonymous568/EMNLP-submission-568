...
def parse_args():
    ...
    parser.add_argument("--vocab", type=str, nargs=3, required=True,
...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        if params.use_pretrained_embedding:
            _, mem_embs = dataset.get_pre_embeddings(params.embeddings)
            features['mem_embs'] = mem_embs
            print('Loaded Embeddings!', mem_embs.shape)
        ...
        for i in range(len(params.device_list)):
            placeholders.append({
                ...
                "source_length": tf.placeholder(tf.int32, [None],
                                                "source_length_%d" % i), 
                "memory": tf.placeholder(tf.int32, [None, None],
                                         "memory_%d" % i),
                "memory_length": tf.placeholder(tf.int32, [None],
                                                "memory_length_%d" % i),
            })
        ...
        for index in range(len(sorted_inputs[0])):
            ...
            restored_memory.append(sorted_inputs[1][sorted_keys[index]])
            ...
            for outputs, scores in zip(restored_outputs, restored_scores):
                for output, score in zip(outputs, scores):
                    ...
                    if not args.log:
                        ...
                        memory = restored_memory[count]
                        values = (count, source, memory, score, decoded)
