...
def _get_inference_fn(model_fns, features):
    def inference_fn(inputs, state):
        local_features = {
            ...
            "memory_length": features["memory_length"],
            # "mem_embs": features["mem_embs"],
            # [bos_id, ...] => [..., 0]
...
def create_inference_graph(model_fns, features, params):
    ...
    source_shape = tf.shape(features["source"])
    ...
    features["source_length"] = tf.tile(features["source_length"],
                                        [1, beam_size])
    source_shape = tf.shape(features["source_length"])
    ...
    # Expand the inputs
    # [batch, length] => [batch, beam_size, length]
    features["memory"] = tf.expand_dims(features["memory"], 1)
    features["memory"] = tf.tile(features["memory"], [1, beam_size, 1])
    memory_shape = tf.shape(features["memory"])

    # [batch, beam_size, length] => [batch * beam_size, length]
    features["memory"] = tf.reshape(features["memory"],
                                    [memory_shape[0] * memory_shape[1], memory_shape[2]])

    # For memory sequence length
    features["memory_length"] = tf.expand_dims(features["memory_length"], 1)
    features["memory_length"] = tf.tile(features["memory_length"],
                                        [1, beam_size])
    memory_shape = tf.shape(features["memory_length"])

    # max_length = features["memory_length"] + decode_length

    # [batch, beam_size, length] => [batch * beam_size, length]
    features["memory_length"] = tf.reshape(features["memory_length"],
                                           [memory_shape[0] * memory_shape[1]])
