...
def encoding_graph(features, mode, params):
    ...
    mvocab = params.vocabulary["memory"]
    mem_vocab_size = len(mvocab)
    if mode != "train":
        mem_embedding = tf.get_variable("memory_embedding",
                                        [src_vocab_size, hidden_size],
                                        initializer=initializer)
    else:
        mem_emb_initializer = tf.constant_initializer(features['mem_embs'])
        mem_embedding = tf.get_variable("memory_embedding",
                                                [mem_vocab_size, hidden_size],
                                                initializer=mem_emb_initializer,
                                                trainable=False)
    mem_inputs = tf.gather(mem_embedding, mem_seq) * (hidden_size ** 0.5)
    mem_inputs = mem_inputs * tf.expand_dims(mem_mask, -1)

    # Preparing encoder
    if params.shared_source_target_embedding:
        text_inputs = tf.nn.bias_add(inputs, bias)
    else:
        text_inputs = inputs
    text_inputs = add_timing_signal(text_inputs)
    encoder_input = tf.concat([text_inputs, mem_inputs], 1)

    enc_attn_bias = attention_bias(tf.concat([src_mask, mem_mask], 1), "masking")
